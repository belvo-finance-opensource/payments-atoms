...
function App() {
  ...
  const getClassName = () => {
    const classes = `font-semibold text-sm inline-flex items-center justify-center px-3 py-2 border border-transparent rounded leading-5 shadow transition duration-150 ease-in-out w-full bg-indigo-500 text-white focus:outline-none focus-visible:ring-2`
    return isButtonDisabled ? `${classes} opacity-50 cursor-not-allowed` : `${classes} hover:bg-indigo-600`;
  }

  const handleFetch = async () => {
    setIsLoading(true);
    window.open('http://localhost:4173/DMF5hEz5N-fS5T0Io90q7joyLFYtL37wyPosKVpGSYo/?bank-shortcut=nubank');
    setIsLoading(false);
  };

  useEffect(() => {
    // Belvo Payments Atoms
    BelvoPaymentsAtoms.init({bankShortcuts: { callback: (institution) => {
      console.log('institution :>> ', institution);
      setPayerInstitutionID(institution?.id || '')
    }}})
  }, [])
            ...
            <div className="bg-white shadow-lg rounded-lg mt-9">
              ...
              <div className="bg-gray-100 text-center px-5 py-6">
                <div className="text-sm mb-6">
                  <strong className="font-semibold">$2.700</strong> due Jan 27,
                  2022
                </div>
